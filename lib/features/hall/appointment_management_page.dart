...
class _AdminApptServicePageState extends State<AdminApptServicePage> {
      ...
      if (mounted) {
        setState(() {
          _services = (data as List)
              .map<_ApptService>(
                (m) =>
                    _ApptService.fromMap(Map<String, dynamic>.from(m as Map)),
              )
              .toList();
          _loading = false;
        });
  ...
  void _goAdd() {
    Navigator.push(
      context,
      instantRoute(const _CreateApptServicePage()),
    ).then((_) => _fetch());
  }
  ...
  void _goEdit(_ApptService svc) {
    Navigator.push(
      context,
      instantRoute(_EditApptServicePage(service: svc)),
    ).then((_) => _fetch());
  }
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (widget.service.duration != null &&
                                widget.service.duration!.isNotEmpty) ...[
                              const SizedBox(height: 5),
                              Container(
                                padding: const EdgeInsets.symmetric(
                                  horizontal: 9,
...
class _CreateApptServicePageState extends State<_CreateApptServicePage> {
  ...
  void dispose() {
    ...
    _descCtrl.dispose();
    for (final f in _residentFields) {
      f.dispose();
    }
    super.dispose();
  }
          ...
          child: Column(
            ...
            children: [
              ...
              _DynamicFieldSection(
                ...
                infoText:
                    'Define fields for the resident\'s personal information '
