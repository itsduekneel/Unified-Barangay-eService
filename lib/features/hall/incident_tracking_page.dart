...
const _statusMap = {
  ...
  'investigating':
  _StatusCfg('Investigating', _kBlue, _kBlueBg, _kBlueBorder, Icons.manage_search_rounded),
  'resolved':
  _StatusCfg('Resolved', _kGreen, _kGreenBg, _kGreenBorder, Icons.check_circle_outline_rounded),
  'dismissed':
  _StatusCfg('Dismissed', _kGray, _kGrayBg, _kGrayBorder, Icons.do_not_disturb_rounded),
};
...
const _severityMap = {
  ...
  'medium':
  _SeverityCfg('Medium', _kOrange, _kOrangeBg, _kOrangeBorder, Icons.remove_rounded),
...
class _IncidentCategory {
  ...
  factory _IncidentCategory.fromMap(Map<String, dynamic> m) => _IncidentCategory(
    id: m['id'].toString(),
    name: m['name'] ?? '',
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          _StatPill(
              '${counts['investigating']}', 'Active', _kBlue, _kBlueBg, _kBlueBorder),
          const SizedBox(width: 8),
          _StatPill(
              '${counts['resolved']}', 'Resolved', _kGreen, _kGreenBg, _kGreenBorder),
          const SizedBox(width: 8),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          runSpacing: 3,
                          children: [
                            _IconText(Icons.person_outline_rounded, incident.reporterName, 11),
                            if (incident.referenceNo != null)
                              _IconText(Icons.sell_outlined, incident.referenceNo!, 11),
                          ],
                        ),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(cfg.label,
              style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: cfg.fg)),
        ],
      ),
    );
  }
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(sevCfg.label,
                  style:
                  TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: sevCfg.fg)),
            ],
          ),
        ),
        const Spacer(),
        Text(incident.createdAt, style: const TextStyle(fontSize: 9, color: _kText3)),
        const SizedBox(width: 8),
        _QuickAction(
          label: 'View',
...
class _AdminIncidentCategoryPageState extends State<AdminIncidentCategoryPage> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _categories = (data as List).map((m) => _IncidentCategory.fromMap(m)).toList();
          _loading = false;
        });
      }
    } catch (_) {
  ...
  void _goAdd() {
    Navigator.push(context, instantRoute(const _CreateIncidentPage()))
        .then((_) => _fetch()); // ← refresh after returning from create
  }

  void _goEdit(_IncidentCategory cat) {
    Navigator.push(context, instantRoute(_EditIncidentCategoryPage(category: cat)))
  ...
  Future<void> _toggle(_IncidentCategory cat, bool val) async {
    ...
    try {
      await _sb
          .from('incident_categories')
          .update({'is_active': val}).eq('id', cat.id);
    } catch (_) {
      // Revert on failure
      setState(() {
        final idx = _categories.indexWhere((c) => c.id == cat.id);
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                _CategoryPill('$total', 'Total', _kAccent, _kAccentBg, _kAccentBorder),
                const SizedBox(width: 8),
                _CategoryPill('$active', 'Active', _kGreen, _kGreenBg, _kGreenBorder),
                const SizedBox(width: 8),
                _CategoryPill('$disabled', 'Disabled', _kGray, _kGrayBg, _kGrayBorder),
              ],
            ),
          ),
          const SizedBox(height: 14),
      ...
      builder: (_, __) {
        ...
        return AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: isOn ? _kBorder : _kBorder.withOpacity(0.45),
            ),
            boxShadow: [
              BoxShadow(
            ...
            child: Column(
              children: [
                    ...
                    children: [
                      ...
                      AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(
                              color: displayColor.withOpacity(isOn ? 0.28 : 0.12)),
                        ),
                        child: Icon(displayIcon,
                            color: displayColor.withOpacity(isOn ? 1.0 : 0.5),
                            size: 24),
                      ),
                      const SizedBox(width: 12),
                ...
                Container(
                  decoration: const BoxDecoration(
                    border:
                    Border(top: BorderSide(color: _kBorder, width: 0.5)),
                  ),
                  padding:
...
class _EditIncidentCategoryPageState extends State<_EditIncidentCategoryPage> {
  ...
  Future<void> _save() async {
    ...
    try {
      await _sb.from('incident_categories').update({
                  ...
                  return Expanded(
                    child: GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        margin: EdgeInsets.only(right: s != _kSeverities.last ? 8 : 0),
                        padding: const EdgeInsets.symmetric(vertical: 10),
                        decoration: BoxDecoration(
...
class _IncidentDetailPageState extends State<_IncidentDetailPage> {
  ...
  Future<void> _delete() async {
    final confirmed =
    await _confirm(context, 'Delete this report?', 'This action cannot be undone.');
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _DetailCard(
              child:
              _DetailRow(Icons.calendar_today_rounded, 'Filed', widget.incident.createdAt),
            ),
            const SizedBox(height: 12),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  value.isEmpty ? '—' : value,
                  style:
                  const TextStyle(fontSize: 12, color: _kText3, height: 1.4),
                ),
              ],
            ),
          ),
                  ...
                  return Expanded(
                    child: GestureDetector(
                      ...
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 150),
                        margin:
                        EdgeInsets.only(right: s != _kSeverities.last ? 8 : 0),
                        padding: const EdgeInsets.symmetric(vertical: 10),
                        decoration: BoxDecoration(
                          color: active ? c.bg : _kSurface,
                          borderRadius: BorderRadius.circular(10),
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child:
                  Icon(opt.icon, size: 20, color: active ? activeColor : _kText3),
                ),
              );
            },
          ),
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight.w700,
                      color:
                      widget.field.isRequired ? _kAccent : _kText2,
                    ),
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
        ...
        children: [
          Padding(
            padding: EdgeInsets.only(
                left: 10, top: maxLines > 1 ? 11 : 0, right: 2),
