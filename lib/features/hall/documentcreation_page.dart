...
class PlacedField {
  ...
  Map<String, dynamic> toMap() => {'id': id, 'label': label, 'x': xPct, 'y': yPct};
...
class DocumentRequest {
  ...
  // ✅ FIX: now reads created_at from Supabase properly
  ...
  // ✅ FIX: toMap does NOT include id/created_at — let Supabase handle those
...
class _DocumentIssuanceState extends State<DocumentIssuance> {
  ...
  // ✅ FIX: proper error handling + clear error state on success
  Future<void> _fetchDocuments() async {
    ...
    try {
      final data = await _supabase
          .from('documents')
          .select(
        'id, title, amount, is_published, icon_code, pdf_name, pdf_url, status, form_fields, created_at',
      )
      ...
      final list = (data as List)
          .map((e) => DocumentRequest.fromMap(Map<String, dynamic>.from(e)))
          .toList();
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              ...
              _fetchDocuments(); // ✅ refresh after creation
            },
          ),
        ],
      ),
...
class _DocumentCreationPageState extends State<DocumentCreationPage> {
  ...
  static const List<String> _commonFields = [
    'Full Name', 'Address', 'Date', 'Contact No.',
    'Date of Birth', 'Civil Status', 'Age', 'Signature',
  ];
  ...
  void initState() {
    ...
    _pdfUrl = doc?.pdfUrl;
    _hasPdf = (_pdfName != null && _pdfName!.isNotEmpty) ||
  ...
  Future<String?> _uploadPdfToStorage(File file, String fileName) async {
    try {
      ...
      await _supabase.storage.from('pdf-templates').upload(
        path,
        file,
  ...
  void _handleUploadPdf() async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        final saved = await file.copy('${dir.path}/${result.files.single.name}');
        setState(() {
          _pdfPath = saved.path;
  ...
  // ✅ FIX: explicit select() after insert to confirm it was saved
  Future<void> _handleSave() async {
    ...
    try {
      ...
      if (_pdfPath != null) {
        ...
        if (uploaded != null) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('PDF upload failed. Check storage bucket permissions.'),
                backgroundColor: Colors.red,
              ),
            );
          }
      ...
      if (_isEditing) {
        ...
        // ✅ Insert new — select() confirms the row was actually created
        ...
        debugPrint('Inserted document id: ${result['id']}');
      }

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isEditing ? 'Document updated!' : 'Document created!'),
            backgroundColor: kPrimary,
          ),
        );
        Navigator.pop(context);
        ...
        actions: [
          _isSaving
            ...
            child: SizedBox(
              ...
              : IconButton(
            icon: const Icon(Icons.save_rounded, color: kPrimary, size: 20),
                ...
                child: SfPdfViewer.network(
                  ...
                  onDocumentLoadFailed: (_) {
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      if (mounted) setState(() { _hasPdf = false; _pdfUrl = null; });
                    });
                  },
                ),
              ),
            if (_hasPdf && _pdfPath == null && _pdfUrl == null) _buildPdfSection(),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: AppColors.primarySuperLight, borderRadius: BorderRadius.circular(8)),
            child: Row(
              children: [
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: const Icon(Icons.text_fields_rounded, color: kPrimary, size: 18),
          ...
          Text('Common Fields — drag onto the PDF', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.grey.shade500)),
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 8, children: _commonFields.map((l) => _DraggableChip(label: l)).toList()),
          if (_customPaletteFields.isNotEmpty) ...[
            const SizedBox(height: 12),
            Text('Your Fields', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.grey.shade500)),
            const SizedBox(height: 8),
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: _customPaletteFields.map((l) => _DraggableChip(label: l, onDelete: () => setState(() => _customPaletteFields.remove(l)))).toList(),
            ),
          ],
          if (_placedFields.isNotEmpty) ...[
            ...
            Row(
              children: [
                const Icon(Icons.check_circle_rounded, color: kPrimary, size: 14),
                const SizedBox(width: 6),
                Text('${_placedFields.length} field(s) placed on PDF', style: const TextStyle(fontSize: 11, color: kPrimary, fontWeight: FontWeight.w600)),
                const Spacer(),
                GestureDetector(
                  onTap: () => setState(() => _placedFields.clear()),
                  child: Text('Clear all', style: TextStyle(fontSize: 11, color: Colors.red.shade400)),
                ),
              ],
            ),
          ],
                  ...
                  Positioned.fill(
                    child: DragTarget<String>(
                      ...
                      builder: (ctx, candidates, _) => candidates.isNotEmpty
                          ? Container(decoration: BoxDecoration(color: kPrimary.withOpacity(0.07), border: Border.all(color: kPrimary.withOpacity(0.35), width: 2)))
                          : const SizedBox.expand(),
                    ),
                  ),
                ..._placedFields.map((field) => _PlacedFieldOverlay(
                  ...
                  onMove: (dx, dy) {
                    setState(() {
                      field.xPct = ((field.xPct * cw + dx) / cw).clamp(0.01, 0.78);
                      field.yPct = ((field.yPct * ch + dy) / ch).clamp(0.01, 0.93);
                    });
                  },
                      ...
                      child: Container(
                        ...
                        child: const Row(
                          ...
                          children: [
                            ...
                            SizedBox(width: 6),
                            Text('Drag fields here to place them', style: TextStyle(color: Colors.white, fontSize: 11)),
                          ],
                        ),
                      ),
                    ),
                ...
                Positioned(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: kPrimary, borderRadius: BorderRadius.circular(20), boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 6, offset: Offset(0, 2))]),
                      child: const Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(Icons.swap_horiz_rounded, color: Colors.white, size: 14),
                          SizedBox(width: 5),
                          Text('Replace', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _buildActionRow() {
    return Container(
      ...
      decoration: const BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 2, offset: Offset(0, 2))]),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [_buildPublishToggle(), const Spacer(), _buildPreviewPill()]),
          const SizedBox(height: 10),
          _buildTextField(label: 'Document Name', icon: Icons.description_outlined, controller: _titleController),
          const SizedBox(height: 8),
          _buildTextField(label: 'Fee', icon: Icons.payments_outlined, controller: _feeController, isNumber: true),
        ],
      ),
    );
  }
      ...
      children: [
        GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            width: 38, height: 22,
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: _isPublished ? kPrimary : Colors.grey.shade300),
            child: AnimatedAlign(
              duration: const Duration(milliseconds: 200),
              alignment: _isPublished ? Alignment.centerRight : Alignment.centerLeft,
              child: Container(margin: const EdgeInsets.all(2), width: 16, height: 16, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle)),
            ),
          ),
        ),
        const SizedBox(width: 6),
        Text(
          ...
          style: TextStyle(color: _isPublished ? const Color(0xFF2E7D32) : const Color(0xFFD32F2F), fontWeight: FontWeight.w600, fontSize: 12),
        ),
      ],
    );
  }
  ...
  Widget _buildPreviewPill() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(16), border: Border.all(color: kPrimary.withOpacity(0.1))),
      child: const Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.remove_red_eye_outlined, size: 14, color: kPrimary),
          SizedBox(width: 4),
          Text("Preview", style: TextStyle(color: kPrimary, fontSize: 12, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }

  Widget _buildTextField({required String label, required IconData icon, required TextEditingController controller, bool isNumber = false}) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 12),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: [
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(10),
                        border: Border(bottom: BorderSide(color: isActive ? kPrimary : Colors.transparent, width: 2.5)),
                      ),
                      child: Center(
                        child: Text(tabLabels[index], style: TextStyle(fontWeight: isActive ? FontWeight.bold : FontWeight.w500, fontSize: 13, color: isActive ? kPrimary : Colors.grey.shade600)),
                      ),
                    ),
                  ),
                );
          ...
          AnimatedCrossFade(
            firstChild: Container(
              ...
              decoration: BoxDecoration(color: Colors.grey.shade50, border: Border(top: BorderSide(color: Colors.grey.shade200), bottom: BorderSide(color: Colors.grey.shade200))),
              child: SingleChildScrollView(
            ...
            crossFadeState: _selectedTab == 2 ? CrossFadeState.showFirst : CrossFadeState.showSecond,
  ...
  Widget _toolbarBtn(IconData icon) {
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () {},
        child: Padding(padding: const EdgeInsets.all(8), child: Icon(icon, size: 20, color: Colors.grey.shade700)),
      ),
    );
  }
        ...
        child: Column(
          children: [
            Container(
              ...
              decoration: const BoxDecoration(color: AppColors.primarySuperLight, shape: BoxShape.circle),
              child: const Icon(Icons.description_outlined, color: AppColors.primary, size: 28),
            ),
            const SizedBox(height: 12),
            const Text('No PDF uploaded', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16, color: AppColors.textDark)),
            const SizedBox(height: 4),
            Text('Tap to attach a PDF template', style: TextStyle(fontSize: 12, color: Colors.grey.shade500), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
...
class _DraggableChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final child = Container(
      ...
      decoration: BoxDecoration(color: AppColors.primarySuperLight, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.primaryLight)),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.drag_indicator_rounded, size: 13, color: kPrimary),
          const SizedBox(width: 4),
          Text(label, style: const TextStyle(fontSize: 12, color: kPrimary, fontWeight: FontWeight.w500)),
          if (onDelete != null) ...[
            const SizedBox(width: 5),
            GestureDetector(onTap: onDelete, child: const Icon(Icons.close_rounded, size: 13, color: kPrimary)),
          ],
        ],
      ),
    );
    ...
    final feedback = Material(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
        decoration: BoxDecoration(color: kPrimary, borderRadius: BorderRadius.circular(8), boxShadow: [BoxShadow(color: kPrimary.withOpacity(0.45), blurRadius: 10, offset: const Offset(0, 4))]),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.drag_indicator_rounded, size: 13, color: Colors.white),
            const SizedBox(width: 5),
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: kPrimary.withOpacity(0.18), blurRadius: 5, offset: const Offset(0, 2))],
          ),
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (editMode) ...[const Icon(Icons.drag_indicator_rounded, size: 11, color: kPrimary), const SizedBox(width: 3)],
              Text(field.label, style: const TextStyle(fontSize: 10, color: kPrimary, fontWeight: FontWeight.w700)),
              if (editMode) ...[
                const SizedBox(width: 5),
                GestureDetector(onTap: onDelete, child: Icon(Icons.close_rounded, size: 11, color: Colors.red.shade400)),
              ],
            ],
          ),
        ),
