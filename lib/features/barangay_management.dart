...
class _Barangay {
  ...
  IconData get displayIcon => iconKey != null && iconKey!.isNotEmpty
      ? _iconByKey(iconKey!)
                    ...
                    ListView.separated(
                      ...
                      itemBuilder: (_, i) => _BarangayCard(
                        ...
                        onTap: () => Navigator.push(
                          context,
                          instantRoute(
                            _BarangayDetailPage(barangay: filtered[i]),
                          ),
                        ),
                      ),
                    ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (barangay.captain != null &&
                            barangay.captain!.isNotEmpty)
                          _IconText(
                            Icons.person_outline_rounded,
                            'Kap. ${barangay.captain!}',
                            11,
                          ),
                        const SizedBox(height: 3),
                        Wrap(
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(
                      ...
                      side: BorderSide(
                        color: b.isActive ? _kGrayBorder : _kGreenBorder,
                      ),
                    ),
                  ),
                ),
...
class _CreateBarangayPageState extends State<_CreateBarangayPage> {
  ...
  final String _selectedIconKey = 'location';
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await _sb.from('barangays').insert({
        ...
        'captain': _captainCtrl.text.trim().isEmpty
            ? null
            : _captainCtrl.text.trim(),
      ...
      if (uid != null && uid.isNotEmpty) {
        try {
          final attrs = AdminUserAttributes(
            email: _emailCtrl.text.trim(),
            password: _newPasswordCtrl.text.isNotEmpty
      ...
      await _sb
          ...
          .update({
            ...
            'captain': _captainCtrl.text.trim().isEmpty
                ? null
                : _captainCtrl.text.trim(),
            'zone': _zoneCtrl.text.trim().isEmpty
                ? null
                : _zoneCtrl.text.trim(),
            'address': _addressCtrl.text.trim().isEmpty
                ? null
